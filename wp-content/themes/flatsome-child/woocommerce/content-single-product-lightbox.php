                ...
                <div class="pizza-size-options">
                    <div class="size-button-group" <?php echo ! $is_pizza ? 'style="display: none !important;"' : ''; ?>>
                        ...
                        <button id="btn-paired" class="size-option" data-size="paired" type="button"
                            <?php echo ! $has_upsells ? 'style="display: none !important;"' : ''; ?>>
                            <img src="<?php echo esc_url( get_site_url() . '/wp-content/uploads/images/pizza-icon-black.png' ); ?>"
                                alt="<?php esc_attr_e( 'Paired pizza', 'flatsome' ); ?>" class="size-icon">
                    ...
                    <div id="pizza-whole" class="pizza-display">
                        ...
                        <div class="pizza-description">
							...
							?>
                            <br>
                            <a href="<?php echo esc_url( get_permalink( $product_id ) ); ?>" class="read-more-toggle">
                    ...
                    <div id="pizza-paired" class="paired-pizza-container">
                        ...
                        <div class="title-section">
                            <h1><?php printf( esc_html__( '%s with', 'flatsome' ), esc_html( get_the_title() ) ); ?>
                            </h1>
                        </div>
								...
								if ( $has_upsells ) {
									...
									if ( ! empty( $paired_products ) ) {
										foreach ( $paired_products as $paired_prod ) {
											...
											?>
                                <div class="pizza-card" data-product-id="<?php echo esc_attr( $paired_id ); ?>"
        ...
        <div class="product-info summary large-6 col entry-summary qv-right-column">
            ...
            <form class="variations_form cart"
				...
				?>
                        ...
                        <div class="checkbox-group">
                            ...
                            <label class="topping-label">
                                <input type="checkbox" value="<?php echo esc_attr( $topping['name'] ); ?>"
                ...
                <div id="paired-pizza-toppings" class="toppings-container" style="display: none;">
                    <!-- Topping Tabs -->
                    <h3 class="topping-tab" data-tab="left-toppings">
                        <?php esc_html_e('Left Pizza Upgrade', 'flatsome'); ?></h3>
                    <h3 class="topping-tab" data-tab="right-toppings">
                        <?php esc_html_e('Right Pizza Upgrade', 'flatsome'); ?></h3>
                    ...
                    <div id="right-toppings" class="half-toppings-section tab-content">
                        <div class="right-toppings-placeholder">
                            <p><?php esc_html_e( 'Please select a pizza from the options below to see available toppings.', 'flatsome' ); ?>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="special-request-section">
                    ...
                    <textarea id="special-request" name="special_request" class="special-request-textarea"
    ...
    $(document).ready(function() {
        ...
        function initSizeToggle() {
            $('#btn-whole').on('click', function() {
                ...
                $('#right-pizza')
                    .attr('src',
                        '<?php echo esc_url( get_site_url() . "/wp-content/uploads/images/half_pizza.png" ); ?>'
                    )
                ...
                success: function(response) {
                    if (response.success && response.data.toppings) {
                        ...
                        if (html) {
                            ...
                            $rightToppings.html(
                                '<div class="right-toppings-placeholder"><p>No toppings available for this pizza.</p></div>'
                            );
                        }
                    } else {
                        $rightToppings.html(
                            '<div class="right-toppings-placeholder"><p>Error loading toppings. Please try again.</p></div>'
                        );
                    }
                },
                error: function() {
            ...
            $(document).off('click', '.pizza-card').off('click.pizzaCard', '.pizza-card')
                .on('click.pizzaCard', '.pizza-card', function() {
                    ...
                    if (!isCurrentlySelected) {
                        ...
                        $('#right-toppings').html(
                            '<div class="right-toppings-placeholder"><p><?php esc_html_e( "Please select a pizza from the options below to see available toppings.", "flatsome" ); ?></p></div>'
                        );
                    }
        ...
        function updateHiddenInputs() {
            ...
            if (isPairedMode) {
                ...
                $('.left-topping:checked').each(function() {
                    ...
                    leftToppings.push({
                        name: $(this).val(),
                        price: isNaN(price) ? 0 : price, // Convert NaN to 0 for free toppings
        ...
        function initAddToCart() {
            ...
            $(document).off('click', 'form.cart button[type="submit"], .single_add_to_cart_button').off(
                ...
                .on('click.addToCart', 'form.cart button[type="submit"], .single_add_to_cart_button',
                    function(e) {
                        ...
                        if ($variationForm.length > 0) {
                            ...
                            // Only validate variation if NOT in paired mode
                            // In paired mode, we might not have a variation selected for the main product
                            if (!isPairedMode && (!variationId || variationId === '0' || variationId === 0)) {
                        ...
                        // In paired mode, ensure required WooCommerce fields are present
                        if (isPairedMode) {
                            // Set form action if empty
                            if (!$form.attr('action') || $form.attr('action') === '') {
                                $form.attr('action', window.location.origin + '/?wc-ajax=add_to_cart');
                            }

                            // Check if product_id input exists, if not create it
                            ...
                            // Check if add-to-cart input exists, if not create it
                            ...
                            // In paired mode, manually submit via AJAX instead of form submission
                            e.preventDefault();
                            e.stopPropagation();

                            // Find the variation_id from the form
                            let variationId = $form.find('input[name="variation_id"]').val();

                            // If no variation_id, try to find it from the selected attributes
                            if (!variationId || variationId === '0') {
                                const variationsData = $form.data('product_variations');
                                const selectedSize = $form.find('select[name="attribute_size"]').val() ||
                                                    $form.find('input[name="attribute_size"]').val();

                                if (variationsData && selectedSize) {
                                    const matchingVariation = variationsData.find(function(v) {
                                        return v.attributes && v.attributes.attribute_size &&
                                               v.attributes.attribute_size.toLowerCase() === selectedSize.toLowerCase();
                                    });

                                    if (matchingVariation) {
                                        variationId = matchingVariation.variation_id;

                                        // Add variation_id to form if it doesn't exist
                            ...
                            // Send AJAX request - Use standard add-to-cart endpoint
                            $.ajax({
                                type: 'POST',
                                ...
                                success: function(response) {
                                    ...
                                    if (response && response.error) {
                                        // Even with error, try to refresh cart
                        ...
                        if (variationsData && variationsData.length) {
                            ...
                            $variationsForm.find('.variation-select-hidden').on('change', function() {
                                ...
                                for (let i = 0; i < variationsData.length; i++) {
                                    ...
                                    for (let attr in v.attributes) {
                                        if (v.attributes[attr] && v.attributes[attr]
                                            .toLowerCase() === selectedValue.toLowerCase()) {
                                            // Match found - update price
                                            mainProductPrice = parseFloat(v.display_price);

                                            // Update left half price for paired mode (same as found_variation handler)
                                            if ($('#btn-paired').hasClass('active') &&
                                                selectedLeftHalf) {
                                                if (selectedLeftHalf.product_id ===
                                                    mainProductId) {
                                                    selectedLeftHalf.price = mainProductPrice /
                                                        2;
                                                }
                                            }
                            ...
                            if (variation.attributes) {
                                Object.keys(variation.attributes).forEach(function(attrKey) {
                                    ...
                                    if (attrValue) {
                                        ...
                                        $('.variation-button[data-attribute="' + attrKey
                                                .replace('attribute_', '') +
